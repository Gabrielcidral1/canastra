"""Benchmark and measure bot quality: blunder scenarios, win rate vs random, and control vs challenger.
...
CONTROL_CONFIG = {"rollouts": AI_TURN_ROLLOUTS, "rollout_max_steps": AI_TURN_ROLLOUT_MAX_STEPS}

# Challenger: avoids early trincas (especially with jokers) and avoids discarding cards we can add to melds.
# Only accept the challenger as better if it wins more games and/or has higher total points.
CHALLENGER_CONFIG = {
...
def run_one_game_control_vs_challenger(
    ...
    Returns (winner_team, team_scores, completed). team_scores keys are 0 and 1 (team indices).
...
def run_control_vs_challenger(
    ...
    """Run control vs challenger head-to-head. Each side plays as team 0 in half the games
    and team 1 in the other half (for fairness). Results are in terms of control vs challenger,
    not team 0 vs team 1.

    Returns dict with: control_wins, challenger_wins, ties, total_points_control, total_points_challenger,
    games_played, avg_point_diff (challenger - control).
    """
    ...
    avg_diff = (total_points_challenger - total_points_control) / games_played if games_played else 0
    return {
...
def run_one_game_bot_vs_random(
    ...
    """Run one game: bot_team uses play_ai_turn, other team uses random. Returns (winner_team, team_scores)."""
    try:
...
def benchmark_win_rate_vs_random(
    ...
    for i in range(num_games):
        winner, scores = run_one_game_bot_vs_random(seed=seed_base + i, bot_team=bot_team)
        if winner == bot_team:
            wins += 1
        if scores:
...
def _blunder_scenario_discard_joker() -> bool:
    """Scenario: discard phase, we have a joker and a low card. Good move: discard the low card, not the joker."""
    engine = Engine(num_players=4)
    engine.start_new_game()
...
def _blunder_scenario_take_useful_discard() -> bool:
    """Scenario: draw phase, we can add the discard pile top to a meld. Good move: take the pile."""
...
def _blunder_scenario_avoid_discard_matching_pile() -> bool:
    """Scenario: discard phase, pile top is 4♠. We have 4♦ and 7♥. Good: prefer discarding 7♥ (don't match pile)."""
    engine = Engine(num_players=4)
    engine.start_new_game()
    engine.current_player_index = 0
    engine.turn_phase = TurnPhase.DISCARD
...
def run_blunder_scenarios() -> dict:
    """Run hand-crafted scenarios where a blunder would be a bad move. Return passed/total."""
    results = []
    results.append(("Don't discard joker when safe alternative", _blunder_scenario_discard_joker()))
    results.append(("Take discard when we can use it", _blunder_scenario_take_useful_discard()))
    results.append(("Prefer discard that doesn't match pile top", _blunder_scenario_avoid_discard_matching_pile()))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Benchmark bot quality: blunder scenarios, win rate vs random, control vs challenger."
    )
    parser.add_argument("--games", type=int, default=5, help="Games for win-rate vs random (default 5)")
    parser.add_argument(
        "--compare-games",
        type=int,
        default=4,
    ...
    if not args.skip_random:
        ...
        elapsed = time.perf_counter() - start
        print(f"   Win rate: {win_rate_result['win_rate']:.0%} ({win_rate_result['wins']}/{win_rate_result['games']})")
        print(f"   Avg point diff (bot - random): {win_rate_result['avg_point_diff']:.0f}")
    ...
    print(f"   Control:   rollouts={ctrl.get('rollouts')}, rollout_max_steps={ctrl.get('rollout_max_steps')}, discourage_early_trinca={ctrl.get('discourage_early_trinca', False)}")
    print(f"   Challenger: rollouts={chal.get('rollouts')}, rollout_max_steps={chal.get('rollout_max_steps')}, discourage_early_trinca={chal.get('discourage_early_trinca', False)}")
    print(f"   Games: {args.compare_games} per side (total {2 * args.compare_games} requested)")
    start = time.perf_counter()
    cmp = run_control_vs_challenger(num_games_per_side=args.compare_games)
    elapsed = time.perf_counter() - start
    if cmp["games_played"] < cmp["games_requested"]:
        print(f"   Completed: {cmp['games_played']}/{cmp['games_requested']} (some games errored or timed out)")
    ...
    print(f"   Time: {elapsed:.1f}s")
    print()
    print("To test an improvement: set CHALLENGER_CONFIG in benchmark_bot.py and re-run. Challenger should have more wins and/or more total points.")
